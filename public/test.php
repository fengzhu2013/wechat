<?php
    ...
    var_dump(json_encode(formatInfo5($arr,false)));
    exit;
    ...
    function formatInfo5($info,$bool = true)
    {
        //多维数组排序
        $info = sortArray($info,'StartTime','strtotime');
        $data = [];
        foreach ($info as $key => &$val) {
            ...
            $name           = $val['Name'];
            $monthAndDay    = $month.'月'.$day.'日';
            $time           = ['SessionId' => $val['SessionId'],'IdStatus' => $val['IdStatus'],'hour' => $hour];
            $i = 0;
            $j = 0;
            $k = 0;
            if (isset($data['Y'])) {
                foreach ($data['Y'] as $key2 => &$val2) {
                    ...
                    if ($val2['Year'] == $year.'年') {
                        foreach ($val2['M'] as $key3 => &$val3) {
                            if ($val3['Month'] == $name) {
                                foreach ($val3['D'] as $key4 => &$val4) {
                                    if ($val4['Date'] == $monthAndDay) {
                                        $val4['T'][] = $time;
                                        $i = 1;
                                    }
                                }
                                if (0 === $i) {
                                    $newDay = ['Date' => $monthAndDay];
                                    $newDay['T'] = [0 => $time];
                                    $val3['D'][] = $newDay;
                                }
                                /*foreach ($val3['D'] as $key4 => &$val4) {
                ...
                if (0 === $k) {
                    //不存在年，新建
                    $T3[0] = $time;
                    $D3[0] = ['Date'  => $monthAndDay,'T' => $T3];
                    $M3[0] = ['Month' => $name,'D' => $D3];
                    $Y3    = ['Year'  => $year.'年','M' => $M3];
                    $data['Y'][] = $Y3;
                }
            } else {
